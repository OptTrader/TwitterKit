...
open class Request: NSObject {
    ...
    open func fetchTweets(handler: @escaping ([Tweet]) -> Void) {
        fetch { results in
            var tweets = [Tweet]()
    ...
    open var requestForOlder: Request? {
        if min_id == nil {
            ...
            return modifiedRequest(
                parametersToChange: [TwitterKey.MaxID : min_id!]
            )
        }
        return nil
    }
    ...
    open var requestForNewer: Request? {
        if max_id == nil {
            ...
            return modifiedRequest(
                parametersToChange: [TwitterKey.SinceID : max_id!],
                clearCount: true
            )
        }
        return nil
    ...
    fileprivate func modifiedRequest(parametersToChange: Dictionary<String, String>, clearCount: Bool = false) -> Request {
        ...
        if clearCount {
            newParameters[TwitterKey.Count] = nil
        }
