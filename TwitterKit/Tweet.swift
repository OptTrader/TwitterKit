...
open class Tweet: NSObject {
    ...
    init?(data: NSDictionary?) {
        ...
        self.media = Tweet.mediaItemsFromTwitterData(
            data?.value(forKeyPath: TwitterKey.Media) as? NSArray)
        self.hashtags = Tweet.mentionsFromTwitterData(
            data?.arrayForKeyPath(
                TwitterKey.Entities.Hashtags),
            inText: text,
            withPrefix: "#"
        )
        self.urls = Tweet.mentionsFromTwitterData(
            data?.arrayForKeyPath(
                TwitterKey.Entities.URLs),
            inText: text,
            withPrefix: "http"
        )
        self.userMentions = Tweet.mentionsFromTwitterData(
            data?.arrayForKeyPath(
                TwitterKey.Entities.UserMentions),
            inText: text,
            withPrefix: "@"
        )
    }
    ...
    fileprivate static func mentionsFromTwitterData(_ twitterData: NSArray?, inText text: String, withPrefix prefix: String) -> [Mention] {
        ...
        for mentionData in twitterData ?? [] {
            if let mention = Mention(
                fromTwitterData: mentionData as? NSDictionary,
                inText: text as NSString,
                withPrefix: prefix
                ) {
...
open class Mention: NSObject {
    ...
    init?(fromTwitterData data: NSDictionary?, inText text: NSString, withPrefix prefix: String) {
        ...
        guard
            let nsrange = text.rangeOfSubstringWithPrefix(
                prefixAloneOrPrefixedMention,
